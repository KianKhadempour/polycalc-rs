use std::{collections::HashMap, fs::{exists, read}};
...
impl Loader {
    pub fn new(data_path: &'static str) -> Result<Self, FileNotFoundError> {
        if (!exists(data_path).expect("something went very wrong trying to check if the file exists")) {
            return Err(FileNotFoundError { file_path: data_path.to_owned() });
        }
    ...
    pub fn load(&self) -> UnitRegistry {
        ...
        let data: HashMap<String, UnitData> = toml::from_slice(&data_file).expect("unable to deserialize unit data");
        ...
        for (i, (k, v)) in data.into_iter().enumerate() {
            ...
            definitions.push(UnitDefinition { id: unit_id, stats, traits: SmallVec::from_vec(v.traits) });
        }

        UnitRegistry { definitions, name_to_id }
    }
}
